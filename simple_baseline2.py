...
def my_function(k_actions, dict_next_clip_action_4, dict_next_clip_action_3, dict_next_clip_action_2,
                dict_next_clip_action_1, entries_train, dict_clips_train, K):
    ...
    return k, {"verb": [[c[0] for c in certified_sequences[i]] for i in range(len(certified_sequences))],
...
def main():
    ...
    with multiprocessing.Pool(processes=40) as pool:
        results = list(tqdm(pool.imap(functools.partial(my_function, dict_next_clip_action_4=dict_next_clip_action_4,
                                                        dict_next_clip_action_3=dict_next_clip_action_3,
                                                        dict_next_clip_action_2=dict_next_clip_action_2,
                                                        dict_next_clip_action_1=dict_next_clip_action_1,
                                                        entries_train=entries_train, dict_clips_train=dict_clips_train,
                                                        K=K), input_actions.items()),
                            total=len(input_actions), desc='Computing f'))
    dict_predictions = {k: v for k, v in results}
